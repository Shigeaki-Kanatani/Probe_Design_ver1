function probe_design_main_Github_ChatGPT(FASTA_dir, Probelist, out_dir, max_probe_number)
...
for i=1:num_of_gene
    ...
    hairpin_type(i) = string(target_row.hairpin);
    ...
    Probe_list = [];
            ...
            [Export_list] = probe_design_version3_HCR_ChatGPT(Fasta_seq_use, hairpin_type(i), start_seq);
...
for i=1:num_of_gene
    ...
    writematrix(Export_list, fullfile(out_dir, ['Probe_list_'  folder_list.name{i} '.csv']));
...
if max_probe_number > 0
    ...
    for i=1:num_of_gene
        ...
        writematrix(Export_list, fullfile(out_dir, 'Reduced', ['Probe_list_' folder_list.name{i} '.csv']));
...
%% Save summary
    Header_table = ["Target_name", "Sequence_length", "Probe_pair_number"];
    ProbeSequence_length_list = probe_pair_number .* 90 ; % one pair length is 45*2 = 90
    Summary_table_pre = [target_list, ProbeSequence_length_list, probe_pair_number];
